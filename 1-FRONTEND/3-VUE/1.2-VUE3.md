...
- (1) Vue2 uses "Object.defineProperty" API to 【 observe 】 the data, and add "dep.depend" and "dep.notify" as getter and setter methods.
---- "dep.depend" is responsible for pushing "render watcher" to the "dep.sub" array when vue is initialized.
---- "dep.notify" is responsible for popping "render watcher" from the "dep.sub" array when data changed, and then execute the "run" methods of the "watcher".
- (2) React use "setState" to change state to implement reactivity.

2. data flow.
- Vue has "two-way" data binding.
- React uses "one-way" data flow. That means data must be 【 passed 】 from the parent component to the child component by "props" 【 attribute 】. and only the parent component can change the "parent". If the child component want to change the props, It must 【 invoke 】 a function attribute from parent component.
